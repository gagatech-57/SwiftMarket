...
const BuyerPage = ({ products, onAddToCart }) => {
  ...
  const [quickViewProduct, setQuickViewProduct] = useState(null); // PDP active product
  const [currentPage, setCurrentPage] = useState(1);

  const itemsPerPage = 16;

  // Scroll to top of window when active page or selected product changes
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [currentPage, quickViewProduct]);
  ...
  // 3. Related Products Recommendations Logic
  const relatedProducts = useMemo(() => {
    if (!quickViewProduct) return [];
    // Get other products (excluding active one)
    const otherProducts = products.filter(p => p.id !== quickViewProduct.id);
    // Find items in same category
    const sameCategory = otherProducts.filter(p => p.category === quickViewProduct.category);
    
    // Slice up to 4, fill with general if fewer
    let slice = sameCategory.slice(0, 4);
    if (slice.length < 4) {
      const remaining = otherProducts.filter(p => p.category !== quickViewProduct.category);
      slice = [...slice, ...remaining].slice(0, 4);
    }
    return slice;
  ...
  return (
    <div className="buyer-container fade-in">
      {quickViewProduct ? (
        /* --- Dedicated Full Screen Product Detail Page (PDP) --- */
        <div className="pdp-container fade-in">
          ...
          <div className="card pdp-details">
            <div className="pdp-grid">
              ...
              <div className="pdp-info">
                <span className="portal-badge buyer" style={{ alignSelf: 'flex-start', marginBottom: '12px' }}>
                  {quickViewProduct.category || 'General'}
                </span>
                <h1 className="pdp-title">{quickViewProduct.name}</h1>
          ...
          {/* Recommendation Loops Grid */}
          {relatedProducts.length > 0 && (
            <div className="pdp-related" style={{ marginTop: '56px' }}>
              <h2 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '24px', textAlign: 'left' }}>
                People Also Liked
              </h2>
              <ProductList 
